...
TOKENSOWNED=`cat $CACHE_FILE | grep $ADDRESS -A 1 | egrep -o "Owned:\s+[\d\.]+" | awk '{print $2}'`
TOKENSSTAKED=`cat $CACHE_FILE | grep $ADDRESS -A 1 | egrep -o "Staked:\s+[\d\.]+" | awk '{print $2}'`
TOKENSDIFF=`echo $TOKENSOWNED-$TOKENSSTAKED | bc`
INDEX=`cat $CACHE_FILE | grep NU | egrep -o "Owned:\s+[\d\.]+" | awk '{print $2}'`
TOKENSTOTAL=`echo $INDEX | xargs | tr ' ' '+' | bc`
INDEX1=`cat $CACHE_FILE | grep -e "Next period confirmed" -e Pending -B3 | egrep -o "Staked:\s+[\d\.]+" | awk '{print $2}' | xargs | tr ' ' '+' | bc`
