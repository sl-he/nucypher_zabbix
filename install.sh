...
crontab -l | { cat; echo "*/5 * * * * su nucypher -c 'cd ~ && source nucypher-venv/bin/activate && nucypher worklock status --network cassandra --provider ~/.ethereum/goerli/geth.ipc --bidder-address $STAKERADDRESS --poa > /tmp/worklock.tmp && cp /tmp/worklock.tmp /tmp/worklock.txt'"; } | crontab -
